...
class Hierarchy:
    def __init__(self, root: Node, node_humanoid_map: Dict[Node, HumanoidBone]) -> None:
        ...
        self.humanoid_node_map: Dict[HumanoidBone, Node] = {v: k for k,
                                                            v in self.node_humanoid_map.items()}
        self.renders = [(node, node.renderer) for node,
                        _ in self.root.traverse_node_and_parent() if node.renderer]
    ...
    def to_skeleton(self) -> Skeleton:
        ...
        left_toes = node_to_joint(
            self[HumanoidBone.leftToes], left_foot)
